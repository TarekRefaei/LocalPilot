...
class FakeNode:
    def __init__(
        self, type_: str, name_text: str | None = None, start=(0, 0), end=(0, 0), children=None
    ):
        self.type = type_
...
@pytest.mark.asyncio
async def test_get_node_type_and_declaration_name_and_make_chunk_id(tmp_path: Path, monkeypatch):
    ch = SemanticChunker()
...
async def test_split_python_class_with_methods(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(ch, "_get_declaration_name", lambda node, fp: (node._name_text or ""))
    ...
    cls = FakeNode("class_definition", name_text="C", start=(1, 0), end=(8, 0), children=[m1, m2])
...
async def test_split_ts_class_fallback_methods(tmp_path: Path, monkeypatch):
    ...
    cls = FakeNode("class_declaration", name_text="A", start=(1, 0), end=(5, 0), children=[body])
