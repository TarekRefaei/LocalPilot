...
import { ValidationCode, isValidActionType } from '../../domain/plan.validation';
...
export function validatePlan(plan: Plan | null | undefined): ValidationWarning[] {
  ...
  if (!plan) {
    warnings.push({ code: ValidationCode.EMPTY_PLAN, message: 'Plan is empty or could not be parsed.' });
    return warnings;
  }

  if (!Array.isArray(plan.tasks) || plan.tasks.length === 0) {
    warnings.push({ code: ValidationCode.NO_TASKS, message: 'Plan has no tasks.' });
    return warnings;
  }
  ...
  plan.tasks.forEach((t: Task, idx: number) => {
    if (!t.filePath) {
      warnings.push({
        code: ValidationCode.MISSING_FILE_PATH,
    ...
    if (!t.actionType) {
      warnings.push({
        code: ValidationCode.MISSING_ACTION_TYPE,
    ...
    } else if (!isValidActionType(t.actionType)) {
      warnings.push({
        code: ValidationCode.INVALID_ACTION_TYPE,
  ...
  for (const t of plan.tasks) {
    if (ids.has(t.id)) {
      warnings.push({ code: ValidationCode.DUPLICATE_TASK_ID, message: `Duplicate task id detected: ${t.id}` });
      break;
    }
    ids.add(t.id);
  }
